...
标准化的 Prompt 设计框架，便于分享和理解：

| 元素 | 全称 | 说明 | 对应 5P |
|-----|------|------|--------|
| **R** | Role | AI 扮演的角色 | Role |
| **A** | Action | 要执行的动作 | Goal |
| **C** | Context | 背景和约束 | Context + Constraints |
| **E** | Expectation | 期望的输出 | Format |
...
metadata:
  ...
  author: user
  license: MIT
  tags: [code-quality, typescript, review]
  description: TypeScript/JavaScript 代码审查
...
## 快速使用
复制到 `.claude/skills/code-review/SKILL.md`
...
| 平台 | 特点 | 适用场景 |
|-----|------|---------|
| **GitHub** | 版本控制、PR 协作 | 团队/开源项目 |
| **Gist** | 快速分享单个 Skill | 临时分享 |
| **FlowGPT** | 社区评分、Remix | 发现新 Skill |
| **PromptHero** | 搜索、分类、评分 | 学习参考 |
| **内部 Wiki** | 权限控制、知识库 | 企业内部 |
...
项目 Skill：
<project>/.claude/skills/  ← 项目专属 Skill
```
...
# 从 URL 安装
claude-skill install https://example.com/skills/code-review.yaml

# 从 GitHub 安装
claude-skill install github:username/repo/skills/code-review
...
Parent（大块，500-2000 tokens）：
└─ 完整章节，提供上下文

Child（小块，100-500 tokens）：
└─ 具体规则/示例，精确匹配
...
## 可用章节（按需加载）
- security: 安全检查（50 条规则）
- performance: 性能优化（30 条规则）
- react: React 专项（40 条规则）
- typescript: TypeScript 专项（35 条规则）

## 快速使用
"审查这段代码" → 自动加载相关章节

## 完整内容
详细规则存储在 chunks/ 目录，按需检索
```
...
**自底向上诊断**：从 Level 1 开始，逐层排查。

### 快速诊断清单

| 层级 | 检查项 | 诊断命令/方法 |
|-----|-------|-------------|
| 加载层 | 文件存在？ | `ls .claude/skills/<name>/SKILL.md` |
| 加载层 | YAML 格式正确？ | 检查 frontmatter 格式 |
| 触发层 | description 包含关键词？ | 检查 description 字段 |
| 触发层 | 是否被其他 skill 抢占？ | 检查同类 skill 的 description |
| 执行层 | 流程步骤完整？ | 对照执行日志检查 |
| 执行层 | 示例代码可运行？ | 手动验证示例 |
| 业务层 | 输出符合预期？ | 用户反馈 |
...
3. 检查 description 覆盖度
   └─ 是否包含用户可能的表述
   └─ 中英文是否都覆盖

4. 检查 Skill 竞争
   └─ 多个 Skill 的 description 是否重叠
   └─ 让目标 Skill 的 description 更精准

解决方案：
- 丰富 description 中的触发词
- 使用 /skill-name 手动触发验证
```

#### Skill 执行不完整

```markdown
症状：Skill 触发了，但流程未完整执行
...
构建健壮的 Skill，优雅处理失败情况。

### 弹性设计原则

```
1. 预期失败（Expect Failure）
   └─ 假设任何步骤都可能失败
...
CLOSED: 正常执行
OPEN: 拒绝执行，直接返回降级结果
HALF-OPEN: 尝试执行，成功则恢复，失败则继续 OPEN
```
...
Level 2: 替代方案（Alternative）
├─ 使用不同工具完成
├─ 简化流程跳过失败步骤
└─ 调用备用 Skill

Level 3: 降级输出（Degraded）
├─ 返回缓存结果
├─ 提供部分功能
└─ 给出最佳努力的建议

Level 4: 安全失败（Safe Fail）
├─ 明确告知用户失败
├─ 提供手动处理指导
└─ 记录问题以便后续修复
```
...
|---------|------|---------|
| 暂时性错误 | 网络抖动、超时 | 重试 |
| 配置错误 | 参数错误、格式问题 | 修正后重试 |
| 依赖错误 | 外部服务不可用 | 降级或替代 |
| 逻辑错误 | Skill 本身的 bug | 修复 Skill |
| 资源错误 | 超出限制 | 排队或拒绝 |
...
减少 Skill 的 token 消耗，提升效率。

### 为什么需要优化

```
问题：
├─ Skill 内容过长 → 消耗大量 context
├─ 加载多个 Skill → context 快速耗尽
└─ 留给用户对话的空间减少
...
| 技术 | 压缩率 | 保真度 | 适用场景 |
|-----|-------|-------|---------|
| **TOON 格式** | 30-50% | 高 | 结构化内容 |
| **语义摘要** | 50-70% | 中 | 长篇描述 |
| **LLMLingua** | 40-60% | 高 | 通用文本 |
| **关键词提取** | 60-80% | 低 | 索引/检索 |
...
在进行代码审查时，安全性检查是最重要的环节。
我们需要检查以下几个方面：

1. SQL 注入：确保所有数据库查询都使用参数化查询
2. XSS 攻击：检查是否有直接插入用户输入到 HTML 的情况
3. 敏感数据：确认没有硬编码的密钥或密码
...
```markdown
# 原始（100 tokens）
当用户请求进行代码审查时，首先需要理解代码的上下文
和目的。这包括阅读相关的文档、PR 描述、以及关联的
issue。理解了这些背景信息后，才能给出有针对性的审查
意见，避免提出与代码目的不符的建议。

# 摘要（30 tokens）
审查前先读 PR 描述和 issue，理解代码目的后再给建议。
```
...
| 指标 | 计算方式 | 告警阈值 |
|-----|---------|---------|
| **触发率** | 触发次数 / 相关请求 | < 70% 需优化 description |
| **成功率** | 成功 / 触发次数 | < 90% 需检查 Skill 质量 |
| **平均耗时** | 总耗时 / 执行次数 | > 30s 需优化或降级 |
| **降级率** | 降级次数 / 触发次数 | > 10% 需检查依赖 |
| **用户满意度** | 正面反馈率 | < 80% 需迭代优化 |
...
### Trace ID: skill-20260113-001

| 步骤 | 时间 | 耗时 | 状态 | 详情 |
|-----|------|------|------|------|
| 请求接收 | 10:00:00.000 | - | ✅ | 用户请求代码审查 |
| Skill 匹配 | 10:00:00.050 | 50ms | ✅ | 匹配 code-review |
| 内容加载 | 10:00:00.100 | 50ms | ✅ | 加载 SKILL.md |
| 执行开始 | 10:00:00.150 | - | ✅ | 开始审查流程 |
| 安全检查 | 10:00:02.150 | 2s | ✅ | 发现 1 个问题 |
| 性能检查 | 10:00:04.150 | 2s | ✅ | 发现 2 个问题 |
| 报告生成 | 10:00:05.150 | 1s | ✅ | 生成审查报告 |
| 完成 | 10:00:05.200 | 50ms | ✅ | 返回结果 |
...
| 级别 | 用途 | 示例 |
|-----|------|------|
| **ERROR** | 执行失败 | Skill 加载失败、依赖不可用 |
| **WARN** | 潜在问题 | 降级触发、超时重试 |
| **INFO** | 正常事件 | Skill 触发、执行完成 |
| **DEBUG** | 调试信息 | 中间结果、决策依据 |

### 日志格式规范

```
[时间] [级别] [Skill名] [TraceID] 消息 {结构化数据}

示例：
[2026-01-13T10:00:00Z] [INFO] [code-review] [trace-001] Skill triggered {"user_request": "审查代码", "match_score": 0.95}
[2026-01-13T10:00:05Z] [INFO] [code-review] [trace-001] Execution completed {"duration_ms": 5200, "issues_found": 3}
[2026-01-13T10:00:10Z] [WARN] [code-review] [trace-002] Fallback triggered {"reason": "timeout", "fallback": "basic_check"}
```
...
### 告警状态
🟢 code-review: 正常
🟡 frontend-daily: 降级率偏高 (15%)
🔴 api-generator: 成功率低 (75%)
...
| 工具 | 特点 | 适用场景 |
|-----|------|---------|
| **Langfuse** | 开源、LLM 专用 | 自托管、隐私敏感 |
| **Braintrust** | 评估 + 追踪 | 需要 A/B 测试 |
| **Maxim AI** | 企业级、完整方案 | 大规模部署 |
| **本地日志** | 简单、无依赖 | 个人/小团队 |
...
| 月份 | 触发 | 成功 | 失败 | 成功率 |
|-----|-----|-----|-----|-------|
| 2026-01 | 500 | 475 | 25 | 95% |

### 查看日志命令
# 查看最近错误
grep "ERROR" .claude/logs/skills.log | tail -20

# 查看特定 Skill 日志
grep "code-review" .claude/logs/skills.log

# 统计成功率
grep "completed" .claude/logs/skills.log | wc -l
```
